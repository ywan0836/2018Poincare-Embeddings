...
if opt.nproc == 0:
    ...
    for rank in range(opt.nproc):
        p = mp.Process(
            target=mytrain.train_mp,
            args=(model, data, optimizer, opt, log, rank + 1, m, queue)
        )
        p.start()
        processes.append(p)
