...
# Calculate the mean rank (Rank) and mean average precision (MAP)
...
parser.add_argument('-dset', help='Dataset to embed', type=str, default='wordnet/noun_closure.tsv')
parser.add_argument('-fout', help='Filename where to store model', type=str, default='nouns.pth')
...
parser.add_argument('-negs', help='Number of negatives', type=int, default=50)
parser.add_argument('-nproc', help='Number of processes', type=int, default=0)
parser.add_argument('-ndproc', help='Number of data loading processes', type=int, default=2)
parser.add_argument('-eval_each', help='Run evaluation each n-th epoch', type=int, default=100)
...
# create adjacency list for evaluation from the dataset
adjacency = ddict(set)
...
# if nproc == 0, run single threaded, otherwise run Hogwild (a paralleling tool)
if opt.nproc == 0:
    train.train(model, data, optimizer, opt, log, 1)
else:
