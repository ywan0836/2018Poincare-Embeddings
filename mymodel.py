...
class SNEmbedding(Embedding):
    ...
    def _forward(self, e):
        o = e.narrow(1, 1, e.size(1) - 1)   # Take out the t terms (t, h)
        s = e.narrow(1, 0, 1).expand_as(o)  # expand t terms for all v
        dists = self.dist()(s, o).squeeze(-1) # Distance between t and h and v's
        return dists
...
class GraphDataset(Dataset):
    ...
    def __init__(self, idx, objects, nnegs, unigram_size=1e8):
        ...
        for i in range(idx.size(0)):
            ...
            self._counts[h] += w
            self._weights[t][h] += w
            #print(self._weights[t][h])
        #self._weights = dict(self._weights)  # Delete by Junbin
